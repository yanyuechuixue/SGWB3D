...
"""
Corrected implementation to reproduce the figures from the paper:
"Subluminal stochastic gravitational waves in pulsar-timing arrays and astrometry"
"""

import numpy as np
import matplotlib.pyplot as plt
from scipy.special import jv, gamma, hyp2f1, legendre
...
class SGWB:
    ...
    This implementation strictly follows the formulas in:
    "Subluminal stochastic gravitational waves in pulsar-timing arrays and astrometry"
    """
...
def plot_power_spectra():
    ...
    plt.tight_layout()
    plt.savefig('figure1_Cl_zz_corrected.png', dpi=300)
    plt.show()
...
def plot_EE_spectra():
    ...
    plt.tight_layout()
    plt.savefig('figure2_Cl_EE_corrected.png', dpi=300)
    plt.show()
...
def plot_BB_spectra():
    ...
    plt.tight_layout()
    plt.savefig('figure3_Cl_BB_corrected.png', dpi=300)
    plt.show()
...
def plot_zE_spectra():
    ...
    plt.tight_layout()
    plt.savefig('figure4_Cl_zE_corrected.png', dpi=300)
    plt.show()
...
def plot_correlation_comparison():
    ...
    plt.tight_layout()
    plt.savefig('figure5_correlation_comparison_corrected.png', dpi=300)
    plt.show()
...
def plot_fR_correlation():
    ...
    for v in velocities:
        ...
        theta, corr = correlation_function('ST', v, 'z', 'z')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('figure6_fR_correlation_corrected.png', dpi=300)
    plt.show()

def correlation_function(alpha, v, X, X_prime, angles=None, l_max=20):
    """
    ...
    # Create SGWB instance
    sgwb = SGWB(l_max=l_max)
    
    # Get power spectrum
    C_l_values = sgwb.C_l(sgwb.ell, X, X_prime, alpha, v)
    
    # Normalize by quadrupole
    C_l_values = sgwb.normalize_spectrum(C_l_values)
