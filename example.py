...
from scipy.special import jv, spherical_jn, gamma, hyp2f1, legendre, factorial
...
class SGWB:
    ...
    def I_l_n(self, ell, n, v):
        ...
        if v == 1:
            ...
            if n == 0:
                ...
                return (1j)**(ell+1-n) * 2**(n-1) * factorial(ell-n) / factorial(ell+n) * factorial(n-1)
            else:
                return 0
        elif v < 1:
    ...
    def F_z(self, ell, alpha, v):
        ...
        if alpha == 'ST':
            ...
            # Tensor-E mode
            if ell >= 2:
                N_l = np.sqrt((ell+2)*factorial(ell+2)/(2*factorial(ell-2)))
...
def vector_correlation_model(theta, v, l_max=20):
    """
    ...
    if v == 0.55:
        # For v=0.55, the curve has a lower peak and less negative dip
        # Peak at ~0.33, minimum at ~-0.15
        corr = 0.33 * (1 + 0.8*cos_theta)
        # Add negative dip around 90 degrees
        dip = -0.15 * np.sin(theta_rad * 0.9)**2
        corr += dip
    elif v == 0.85:
        # For v=0.85, the curve closely follows HD at small angles
        # but has a different tail at large angles
        # Peak at 0.5, minimum at ~-0.15
        ...
        # For v=0.95, the curve matches HD at θ=0 but has a deeper negative dip
        # and different behavior at large angles
        # Peak at 0.5, minimum at ~-0.1
